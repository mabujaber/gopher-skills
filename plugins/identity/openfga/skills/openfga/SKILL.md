...
name: openfga
description: >
  Use when authoring, reviewing, or refactoring OpenFGA authorization models — defining types and
  relations, writing relationship tuples, deriving can_* permissions, applying type restrictions and
  usersets, or authoring .fga / .fga.yaml check, list_objects, and list_users tests. Covers OpenFGA
  SDK integrations in JavaScript, TypeScript, Go, Python, Java, and .NET. Triggers on ".fga model
  files", "OpenFGA permissions", "relationship tuples", "Zanzibar authorization", "ReBAC", "FGA
...
The complete guide is split across the `references/` files indexed above — read the file matching
the rule or topic you need. For SDK integration tasks, consult the `references/sdk-*.md` file for
your language.
