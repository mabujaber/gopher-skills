...
name: k6
description: >
  Use when writing, debugging, or running k6 load/performance tests in JavaScript or TypeScript —
  including test scripts, test types (load/stress/spike/soak/smoke/breakpoint), thresholds, checks,
  scenarios, executors, custom metrics, browser tests, result analysis, k6 Cloud execution, and CI/CD
  integration. Also covers the end-to-end website performance-testing workflow (record → functional →
  hybrid load → report). Triggers on "k6", "load test", "stress test", "performance test", "spike test",
...
## Website Performance Testing (end-to-end workflow)

For performance-testing a public website end to end — record real user flows with Playwright,
build functional protocol + browser tests, then hybrid load tests (one file per test type) with
SLO-backed thresholds and a load-generator monitor — follow the opinionated workflow in
